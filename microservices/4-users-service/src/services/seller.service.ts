...
import { IOrderMessage, IRatingTypes, IReviewMessageDetails, ISellerDocument } from '@thejuggernaut01/jobberapp-shared';
...
const createSeller = async (sellerData: ISellerDocument): Promise<ISellerDocument> => {
  ...
  await updateBuyerIsSellerProp(createdSeller.email as string);
...
const updateSellerCompletedJobsProps = async (data: IOrderMessage) => {
...
const updateSellerReview = async (data: IReviewMessageDetails) => {
